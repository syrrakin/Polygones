...
namespace Polygones
{
    internal class Solver
    {
        ...
        public IEnumerable<Polygone> FindVisiblePolygones(IEnumerable<Polygone> polygones)
        {
            var bag = new ConcurrentBag<Polygone>();
            Parallel.ForEach(polygones, p => { if (CheckPolygone(p)) { bag.Add(p); } });
            return bag;
        }
        private bool CheckPolygone(Polygone polygone)
        {
            //Принимаем, что полигон по условию задачи состоит как минимум из одного отрезка и не проверяем вырожденные случаи
            for (int i = 0; i < polygone.Points.Count - 1; i++)
            {
                if (CheckLine(polygone.Points[i], polygone.Points[i + 1]))
                {
                    return true;
                }
            }
            ...
            return polygone.Points.Count >= 2 ? CheckLine(polygone.Points[0], polygone.Points[polygone.Points.Count - 1]) : false;
        }


        private bool CheckLine(Point point1, Point point2)
        {
            var x1 = point1.X;
            var y1 = point1.Y;
            var x2 = point2.X;
        ...
        private int GetCode(double x, double y)
        {
